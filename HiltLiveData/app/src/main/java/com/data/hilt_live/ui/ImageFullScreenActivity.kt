...
import com.data.hilt_live.adapters.GalleryFullScreenAdapter
import com.data.hilt_live.adapters.SnapOnScrollListener
...
class ImageFullScreenActivity : AppCompatActivity(), SnapOnScrollListener.onSnapPositionChangeListener {
    ...
    override fun onSnapChanged(position: Int) {
        pos=position
        if (imageModels!=null&&position<imageModels?.size!!){
            when (pos) {
                0 -> {
                    next_iv.visibility = View.VISIBLE
                    prev_ic.visibility = View.GONE
